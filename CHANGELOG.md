...
- **Loot roll frame priority text is bigger, and reuses RCLootCouncil_wowaudit's empty space when it's not installed.** The "Prio: 2nd" text `lootFrame.lua` shows for each loot roll entry is now `GameFontNormal` instead of `GameFontNormalSmall`. When `RCLootCouncil_wowaudit` is loaded, it still renders as its own line below the entry, same as before (that addon owns the inline itemLvl line with its own wishlist annotation). When it's *not* loaded, the priority text is appended directly onto that itemLvl line instead -- the same technique wowaudit itself uses -- rather than adding a second line no one else needed the space for.
