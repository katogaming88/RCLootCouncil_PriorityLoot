...
**`priority`** (optional) — item-centric lookup. Keyed by item ID string; value is a per-track object with `H` (Heroic) and/or `M` (Mythic) keys, each an ordered array of `Name-Realm` strings. Heroic and Mythic share the same item ID in this game (the track only changes item level, not the ID), so a single flat list can't tell them apart — the addon resolves the track per lookup, primarily from the dropped item's own live item level (`C_Item.GetDetailedItemLevelInfo`, compared against this tier's known Heroic/Mythic ilvl values), falling back to the live raid's difficulty via `GetInstanceInfo()` if the item level isn't cached yet. Item-level detection works everywhere, including `/rc test`, which never actually places you in a raid instance. When a `priority` entry exists for the dropped item it takes precedence over the player's `players` BiS list. Players absent from the relevant track's list display as **N/A**; if neither signal can resolve a track, the column shows an "unknown raid difficulty" message instead of guessing.
