...
**`priority`** (optional) — item-centric lookup. Keyed by item ID string; value is a per-track object with `H` (Heroic) and/or `M` (Mythic) keys, each an ordered array of `Name-Realm` strings. Heroic and Mythic share the same item ID in this game (the track only changes item level, not the ID), so a single flat list can't tell them apart — the addon reads the live raid's difficulty via `GetInstanceInfo()` at vote/award time and looks up the matching sub-list. When a `priority` entry exists for the dropped item it takes precedence over the player's `players` BiS list. Players absent from the relevant track's list display as **N/A**; if the raid's difficulty can't be determined (not in a raid, or an unmapped difficulty like LFR), the column shows an "unknown raid difficulty" message instead of guessing.
